      ...
      <div style={{ background:"#0d1117", border:`1px solid ${accent}25`, borderRadius:24, width:"100%", maxWidth:560, maxHeight:"90vh", overflowY:"auto", animation:"fadeUp 0.3s ease" }}>
        <div style={{ padding:"20px 24px 0", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            ...
            <div style={{ fontSize:18, fontWeight:800, color:"#fff" }}>{stepTitles[step]}</div>
          </div>
        ...
        <div style={{ padding:"14px 24px 0" }}>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
            ...
            <span style={{ fontSize:11, color:accent }}>{Math.round(progress)}%</span>
          </div>
        ...
        <div style={{ padding:"20px 24px 24px" }}>
          {current.tiles && (
            <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:12 }}>
              {current.tiles.map(ti => (
                <button key={ti.id} onClick={() => { setTile(ti.title); update("type_projet", ti.title); }} style={{ background: tile===ti.title ? `${accent}18` : "rgba(255,255,255,0.03)", border:`1px solid ${tile===ti.title ? accent : "rgba(255,255,255,0.07)"}`, borderRadius:14, padding:"14px 12px", cursor:"pointer", textAlign:"left", fontFamily:"inherit", transition:"all 0.2s" }}>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", marginTop:24, gap:12 }}>
            ...
            {step < steps.length-1 ? (
              <button onClick={() => setStep(p=>p+1)} style={{ background:`linear-gradient(135deg,${accent},${accent}AA)`, color:"#fff", border:"none", padding:"11px 24px", borderRadius:10, fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>{t.next}</button>
            ) : (
  ...
  return (
    <div style={{ fontFamily:"'Outfit',sans-serif", background:"#050810", color:"#e2e8f0", overflowX:"hidden" }}>
      ...
      <section style={{ minHeight:"100svh", display:"flex", alignItems:"center", justifyContent:"center", padding:"100px 24px 60px", position:"relative", overflow:"hidden" }}>
        <div style={{ position:"absolute", inset:0, background:"radial-gradient(ellipse 80% 60% at 50% 0%,rgba(245,158,11,0.07) 0%,transparent 70%)" }} />
        ...
        <div style={{ maxWidth:1100, margin:"0 auto" }}>
          ...
          <div style={{ display:"flex", gap:10, justifyContent:"center", flexWrap:"wrap", marginBottom:40 }}>
            {[["all",t.filter_all],["logos",t.filter_logos],["affiches",t.filter_affiches],["videos",t.filter_videos]].map(([key,lbl]) => (
              <button key={key} onClick={() => setFilter(key)} style={{ background: filter===key ? "linear-gradient(135deg,#f59e0b,#ef4444)" : "rgba(255,255,255,0.05)", color: filter===key ? "#fff" : "#94a3b8", border: filter===key ? "none" : "1px solid rgba(255,255,255,0.07)", padding:"8px 20px", borderRadius:50, fontSize:13, fontWeight: filter===key ? 700 : 400, cursor:"pointer", fontFamily:"inherit", transition:"all 0.2s" }}>{lbl}</button>
            ))}
          </div>
              ...
              <div key={i} className="card" style={{ borderRadius:16, overflow:"hidden", background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", opacity:portIn?1:0, transform:portIn?"none":"translateY(30px)", transition:`all 0.6s ease ${i*0.1}s` }}>
                ...
                <div style={{ padding:"16px 18px" }}>
                  ...
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                    {p.tags.map(tag => <span key={tag} style={{ fontSize:11, background:"rgba(245,158,11,0.1)", color:"#f59e0b", border:"1px solid rgba(245,158,11,0.2)", borderRadius:20, padding:"2px 8px" }}>{tag}</span>)}
                  </div>
                </div>
              </div>
            ))}
      ...
      <section ref={servRef} style={{ padding:"100px 24px", background:"#050810" }}>
        <div style={{ maxWidth:1100, margin:"0 auto" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))", gap:24 }}>
            {t.services.map((s, i) => (
              <div key={i} className="card" style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:24, padding:"32px 28px", opacity:servIn?1:0, transform:servIn?"none":"translateY(40px)", transition:`all 0.7s ease ${i*0.15}s` }}>
      ...
      <section ref={procRef} style={{ padding:"100px 24px", background:"linear-gradient(180deg,#050810,#07101f)" }}>
        <div style={{ maxWidth:1100, margin:"0 auto" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:20 }}>
            {t.steps.map((s, i) => (
              <div key={i} className="card" style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:20, padding:"28px 22px", opacity:procIn?1:0, transform:procIn?"none":"translateY(30px)", transition:`all 0.6s ease ${i*0.1}s` }}>
      ...
      <section style={{ padding:"100px 24px", textAlign:"center", position:"relative", overflow:"hidden", background:"#050810" }}>
        ...
        <div style={{ position:"relative", zIndex:1 }}>
          ...
          <div style={{ display:"flex", gap:14, justifyContent:"center", flexWrap:"wrap" }}>
            <button onClick={() => setModal("graphic")} style={{ background:"linear-gradient(135deg,#f59e0b,#ef4444)", color:"#fff", border:"none", padding:"14px 32px", borderRadius:50, fontSize:15, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>{t.cta_graphic}</button>
            <button onClick={() => setModal("video")} style={{ background:"linear-gradient(135deg,#a855f7,#7c3aed)", color:"#fff", border:"none", padding:"14px 32px", borderRadius:50, fontSize:15, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>{t.cta_video}</button>
          </div>
        </div>
      </section>
...
const label = (color) => ({ display:"inline-block", background:`${color}10`, border:`1px solid ${color}30`, borderRadius:50, padding:"5px 16px", fontSize:11, color, letterSpacing:"2px", textTransform:"uppercase", marginBottom:16 });
