...
function WebBriefModal({ onClose, lang }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(3,7,18,0.85)", backdropFilter:"blur(12px)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ background:"#0d1117", border:"1px solid rgba(0,200,255,0.15)", borderRadius:24, width:"100%", maxWidth:580, maxHeight:"90vh", overflowY:"auto", boxShadow:"0 20px 80px rgba(0,0,0,0.6)" }}>
        <div style={{ padding:"24px 24px 0", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            ...
            <div style={{ fontSize:18, fontWeight:800, color:"#fff", letterSpacing:"-0.5px" }}>{t.form_steps_titles[step]}</div>
          </div>
        ...
        <div style={{ padding:"14px 24px 0" }}>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
            ...
            <span style={{ fontSize:11, color:"#00C8FF", fontWeight:700 }}>{Math.round(progress)}%</span>
          </div>
        ...
        <div style={{ padding:"20px 24px 24px" }}>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", marginTop:24, gap:12 }}>
            ...
            {step < WEB_FORM_STEPS.length-1 ? (
              <button onClick={() => setStep(p=>p+1)} style={{ background:"linear-gradient(135deg,#00C8FF,#a855f7)", color:"#030712", border:"none", padding:"11px 24px", borderRadius:12, fontSize:14, fontWeight:800, cursor:"pointer", fontFamily:"inherit" }}>{t.next}</button>
            ) : (
  ...
  return (
    <div style={{ fontFamily:"'Outfit',sans-serif", background:"#030712", color:"#e2e8f0", overflowX:"hidden" }}>
      ...
      <section style={{ minHeight:"100svh", display:"flex", alignItems:"center", justifyContent:"center", padding:"110px 24px 60px", position:"relative", overflow:"hidden" }}>
        <div style={{ position:"absolute", inset:0, background:"radial-gradient(ellipse 80% 60% at 50% 0%,rgba(0,200,255,0.06) 0%,transparent 70%)" }} />
        <div style={{ position:"absolute", top:"20%", left:"5%", width:400, height:400, background:"radial-gradient(circle,rgba(168,85,247,0.05),transparent 70%)", filter:"blur(60px)", borderRadius:"50%" }} />
        <div style={{ position:"absolute", inset:0, backgroundImage:"linear-gradient(rgba(0,200,255,0.015) 1px,transparent 1px),linear-gradient(90deg,rgba(0,200,255,0.015) 1px,transparent 1px)", backgroundSize:"64px 64px" }} />
        <div style={{ position:"relative", zIndex:1, textAlign:"center", maxWidth:760 }}>
          <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(0,200,255,0.05)", border:"1px solid rgba(0,200,255,0.2)", borderRadius:50, padding:"7px 18px", fontSize:12, color:"#00C8FF", marginBottom:32, fontWeight:600 }}>
          ...
          <button onClick={() => setModal(true)} style={{ background:"linear-gradient(135deg,#00C8FF,#a855f7)", color:"#030712", border:"none", padding:"14px 36px", borderRadius:50, fontSize:16, fontWeight:800, cursor:"pointer", fontFamily:"inherit", boxShadow:"0 8px 30px rgba(0,200,255,0.3)" }} className="primary-web-cta">{t.cta_start}</button>
        ...
        <div style={{ maxWidth:1100, margin:"0 auto" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))", gap:20 }}>
            {WEB_PORTFOLIO.map((p, i) => (
              <div key={i} className="card" style={{ borderRadius:16, overflow:"hidden", background:"rgba(255,255,255,0.015)", border:"1px solid rgba(255,255,255,0.05)", opacity:portIn?1:0, transform:portIn?"none":"translateY(30px)", transition:`all 0.6s ease ${i*0.1}s`, backdropFilter:"blur(8px)" }}>
                ...
                <div style={{ padding:"16px 18px" }}>
                  ...
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                    {p.tags.map(tag => <span key={tag} style={{ fontSize:11, background:`${p.color}10`, color:p.color, border:`1px solid ${p.color}25`, borderRadius:20, padding:"2px 8px" }}>{tag}</span>)}
                  </div>
                </div>
              </div>
            ))}
      ...
      <section ref={servRef} style={{ padding:"100px 24px", background:"#030712" }}>
        <div style={{ maxWidth:1100, margin:"0 auto" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))", gap:24 }}>
            {t.services.map((s, i) => (
              <div key={i} className="card" style={{ background:"rgba(255,255,255,0.015)", border:"1px solid rgba(255,255,255,0.05)", borderRadius:24, padding:"28px 24px", opacity:servIn?1:0, transform:servIn?"none":"translateY(40px)", transition:`all 0.7s ease ${i*0.12}s`, backdropFilter:"blur(8px)" }}>
                ...
                <ul style={{ listStyle:"none", padding:0, margin:0, display:"flex", flexDirection:"column", gap:10 }}>
                  {s.items.map(item => (
                    <li key={item} style={{ display:"flex", alignItems:"center", gap:8, fontSize:12, color:"#94a3b8" }}>
                      <span style={{ width:16, height:16, borderRadius:"50%", background:"rgba(0,200,255,0.08)", border:"1px solid rgba(0,200,255,0.18)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, color:"#00C8FF", flexShrink:0 }}>✓</span>
                      {item}
                    </li>
                  ))}
                </ul>
                <button onClick={() => setModal(true)} style={{ width:"100%", marginTop:20, padding:"12px", borderRadius:12, border:"1px solid rgba(0,200,255,0.18)", background:"rgba(0,200,255,0.04)", color:"#00C8FF", fontWeight:800, fontSize:13, cursor:"pointer", fontFamily:"inherit", transition:"all 0.25s" }} className="services-action-btn">{lang==="en"?"Get started →":"Démarrer →"}</button>
              </div>
            ))}
          </div>
        </div>
            ...
            {t.steps.map((s, i) => (
              <div key={i} style={{ display:"flex", gap:20, opacity:procIn?1:0, transform:procIn?"none":"translateY(20px)", transition:`all 0.6s ease ${i*0.1}s` }}>
                <div style={{ display:"flex", flexDirection:"column", alignItems:"center" }}>
                  ...
                  {i < t.steps.length-1 && <div style={{ width:2, flex:1, background:"rgba(0,200,255,0.08)", margin:"4px 0", minHeight:44 }} />}
                </div>
      ...
      {/* CTA */}
      <section style={{ padding:"100px 24px", textAlign:"center", position:"relative", overflow:"hidden", background:"#050917" }}>
        <div style={{ position:"absolute", top:"50%", left:"50%", transform:"translate(-50%,-50%)", width:600, height:300, background:"radial-gradient(ellipse,rgba(0,200,255,0.06),transparent 70%)", filter:"blur(40px)" }} />
        <div style={{ position:"relative", zIndex:1 }}>
          ...
          <button onClick={() => setModal(true)} style={{ background:"linear-gradient(135deg,#00C8FF,#a855f7)", color:"#030712", border:"none", padding:"16px 40px", borderRadius:50, fontSize:16, fontWeight:800, cursor:"pointer", fontFamily:"inherit", boxShadow:"0 8px 30px rgba(0,200,255,0.25)" }} className="primary-web-cta">{t.cta_start}</button>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:20, marginTop:28, flexWrap:"wrap" }}>
            {[t.devis, t.no_commit, t.support].map(txt => <span key={txt} style={{ fontSize:13, color:"#4b5563", fontWeight:500 }}>{txt}</span>)}
          </div>
        </div>
      </section>
...
const lbl = (color) => ({ display:"inline-block", background:`${color}10`, border:`1px solid ${color}30`, borderRadius:50, padding:"5px 16px", fontSize:11, color, letterSpacing:"2px", textTransform:"uppercase", marginBottom:16, fontWeight:700 });
...
const CSS = `
  ...
  input:focus, textarea:focus, select:focus { border-color: rgba(0,200,255,0.3) !important; outline:none; background: rgba(255,255,255,0.06) !important; }
  textarea { resize:vertical; }
  button:hover { opacity:.88; }
  a:hover { opacity:.88; }

  .primary-web-cta:hover {
    transform: translateY(-2px);
    box-shadow: 0 10px 30px rgba(0,200,255,0.35) !important;
  }
